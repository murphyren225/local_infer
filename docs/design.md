...
本设计的一个特色：**向集群加一台设备，等价于向某个池加一个模型副本**。
已有设备的配置不改，服务不停，从下一个请求起生效；撤下一台设备同样不需要操作。
当前版本把范围限定在三类硬件，不引入更复杂的机制。

加入流程：

1. 新设备执行一次安装命令（首台 `init` 成为 Hub，其余 `join <Hub> --token`）；
2. 设备自行探测硬件，按 §3 的规则确定所属池，启动匹配的引擎与模型，向 Hub 提交节点档案；
3. 控制平面按新的节点集合重新生成路由表，网关秒级重载。

撤出是对称的：设备关机或断网后，控制平面在两个探测周期内判定其失效并从路由表
摘除，该池流量改走幸存节点或云端；设备恢复后自动重新注册归队。

当前支持的硬件与定池结果：

| 硬件 | 自动分配的池 | 状态 |
|---|---|---|
| NVIDIA 24GB 独显机 | 强池（32B），卡上分片兼弱池 | 已验证 |
| 无独显的常开设备（Mac、小主机） | 弱池，可兼 Hub | 已验证 |
| 128GB 统一内存主机 | 长上下文池 | 规划 |

明确不在当前范围内：同一池内多台同型设备的负载分摊（需在池前增加负载均衡层，
Switchyard 每池只指向一个地址）；在途请求向新设备迁移（引擎不支持 KV 跨机迁移，
也无必要）；设备自动发现与网页批准（当前以口令加入）。

实现状态：`init` 与远端 GPU 接入（`link-gpu`）已真机验证；通用 `join`
（任意设备自探测、自定池、自注册）为下一步工作，所需机制均已存在。
...
| 调度层 → 资源层 | HTTP | 独立进程 | 通常分机；同机时按 §7.3 隔离 |
| 调度层 → 云端 | HTTPS | 外部服务 | 外网 |

### 7.2 接入层（harness）的两条放置约束
