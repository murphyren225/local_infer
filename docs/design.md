...
实线 = **数据面**（每个请求都走的路径，编号 1–7）；
虚线 = **控制面**（只在节点生死变化时动作，编号 a–e）。
并发标注：✅ 支持高并发 ｜ ⚠️ 支持但占容量 ｜ ❗ 当前不支持（排队串行）。

```mermaid
flowchart TB
    U((User)) -- 1 --> ACCESS
    subgraph ACCESS["接入层 ✅ 异步转发,无并发压力"]
        W1["网页控制台 :6006"]
        W2["Pi CLI(终端 agent)"]
        W3["API 客户端(公司现有软件)"]
    end
    ACCESS -- "2 OpenAI 格式" --> G["网关 :4000 Switchyard<br/>✅ 无状态异步代理<br/>(再高吞吐用 --workers 多进程)"]
    G -- "3 裁决" --> J["判定器(复用弱池模型)<br/>⚠️ 每次裁决占一次弱池调用<br/>容量规划 +10~20%"]
    G -- "4.a 默认路径" --> WP["弱池节点<br/>GPU 版 vLLM ✅ 批处理 4 路<br/>Mac llama.cpp ❗ 当前单槽位,排队"]
    G -- "4.b 判定超纲,单向升级" --> SP["强池节点 vLLM 32B<br/>✅ 连续批处理,实测 1–2 路满上下文<br/>超载=排队降速,不报错"]
    G -- "4.c 降级模式才走" --> CL["云端 API<br/>✅ 供应商侧近乎无限"]
    WP -- 5 --> G
    SP -- 5 --> G
    CL -- 5 --> G
    G -- "6/7 响应,model 标注实际执行者" --> ACCESS

    subgraph CTRL["控制平面(不在数据路径上,单线程即可,负载与用户量无关)"]
        WD["看门狗(10s 周期)"]
        GR["路由编译器 gen_routes"]
        RT[("路由表 yaml")]
    end
...
- **1** 用户动作，三个入口任选：网页聊天框 / **Pi**（终端里的 agent，接入层的 CLI 形态）/ 程序直接调 API
- **2** 接入层包装成 OpenAI 格式（`model:"auto"` + `messages`）发给网关
- **3** 网关路由：查会话粘性表 → 未升级的会话由判定器读对话轨迹打分
- **4.a** 默认走弱池 ｜ **4.b** 判定确认超纲走强池（会话单向固定）｜ **4.c** 路由表处于降级模式时走云端
- **5→7** 响应原路返回，`model` 字段如实标注实际执行者

**两个平面怎么连接**（原来两张图的关系）：

1. **看的是同一批节点**：控制面虚线 a 探测的 `/health`，和数据面 4.a/4.b 打的是同一组服务——一个问「你活着吗」，一个送活干；
2. **控制面的输出决定数据面的流向**：虚线 c/d 重写的路由表，正是第 4 步「往哪发」的依据。节点死了 → 路由表被改写 → 数据面的 4.a/4.b 自动改道（降级）；节点复活 → 对称还原；
3. **两个平面不共享进程路径**：故障处理再忙也不会阻塞任何请求；反过来请求再多，控制面 10 秒一拍的节奏不变。这就是把两张图分开画的原因——它们逻辑上是两个独立循环，只通过「路由表」这一个文件耦合。
