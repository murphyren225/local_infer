...
| 设备 / 池 | 聚合吞吐 | 每分钟（200 token 答案） | 每小时 |
|---|---|---|---|
| 4090D 强池（32B） | 38 tok/s | 约 11 | 约 660 |
| 4090D 弱池分片（1.7B，vLLM） | 108 tok/s | 约 32（扣判定器开销后约 27） | 约 1600 |
| Mac i9 弱池（1.7B，llama.cpp） | 14 tok/s | 约 4 | 约 250 |

一台标准盒每小时约 2200 个 200 token 级回答；产能与回答长度成反比，
