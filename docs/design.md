...
**请求头**（可选，个人端本机服务自动补齐；第三方客户端不带则退化为无粘性、无任务类型）：

| 头 | 说明 |
|---|---|
| `x-switchyard-session-id` | 会话标识。同一会话回同一副本（前缀缓存）并承载升级粘性；缺省由本机服务按对话首条消息推导 |
| `x-task-type` | 任务类型（`code` / `edit` / `qa` / `extract` / `agent`，可扩展）。供决策器、输出约束与 drafter 选择使用；缺省由网关分类 |
...
按用法再分一档：24GB 上 32B 只能给 5K 上下文，够"单次专家任务"，**不够 agent 多轮工具循环**
（几轮就触发压缩）。agent 用法的建议是 24GB 档强池换 14B 级模型开 32K 窗口，或直接上
32–48GB 档跑 32B；投机解码的 drafter 头再占 1–2GB，做档位预算时一并计入。
...
> 2026-09-26 对照 agent 推理栈的新目标（见 `docs/agent-stack-draft.md`）逐节复核本文档，
> 列出当前设计没有覆盖或已被实践推翻的地方。每条标明影响、改在哪一节、状态。
> 本部分是工作清单，不是承诺；条目关闭后移入对应章节并从这里删除。

## 1. 决策与路由

| # | 缺口 | 影响 | 改哪 | 状态 |
|---|---|---|---|---|
| 1 | 只有"事后判定"（小模型答完再看轨迹），没有"事前决策器"抽象；任务类型、要不要改写、输出长度都没有决策点 | agent 请求先浪费一次小模型调用；无法按任务类型做任何差异化 | 第一部分 §2 第 3 步改为"决策器 → 网关"两段；第三部分 §3.3 增加决策器接口（Jev / AnyJev / 自训 RL 同一接口） | 草案已定，未落地 |
| 2 | 每池只有一个在役节点，加设备只做故障转移；没有池内分发层、没有按吞吐加权、没有会话粘性到副本 | 异构集群加机器不增产能；前缀缓存被打散 | 第一部分 §4 新增"池内分发"节：候选 = 有该模型的副本，排序 = 预计完成时间（等待 + 本条耗时，速度实测），会话粘性优先，PAIR 式本地预留与有序故障转移 | 算法已选定（PAIR 源码 + 预计完成时间），未实现 |
| 3 | 网关外壳是裸 Switchyard；鉴权、配额、预算、审计一直列为"非目标/已知缺口" | 企业交付过不了门槛；云端无预算护栏 | 第三部分 §3.1 改为 LiteLLM Proxy 外壳 + 策略插件，Switchyard 退为插件内的判定实现；第二部分加鉴权头；第一部分 §8 非目标相应收窄 | 选型已定，未实现 |
| 4 | 决策器若用外部 API（Jev），请求摘要出网，与"数据不出内网"（第三部分 §8.3）冲突 | 隐私策略缺一条 | §8.3 增加"决策摘要外发"策略：默认关闭、最小化摘要、按客户开关；本地决策器（AnyJev / 自训）必须始终可用 | 待写 |
| 5 | `confirmations` 文档与代码曾不一致（2 vs 1）；单发请求（`ask`）永远不会升级 | 单次难任务被小模型糊弄 | §3.3 明确：单发请求的升级由事前决策器负责，事后判定只管多轮会话 | 代码已改为 2；语义待补 |

## 2. 推理与速度

| # | 缺口 | 影响 | 改哪 | 状态 |
|---|---|---|---|---|
| 6 | 投机解码完全没写：drafter 类型、领域专用头的训练数据、显存预算、按任务选实例 | 速度路线图缺主项 | 第三部分 §4 新增 §4.7 投机解码规范（n-gram / STANDALONE / EAGLE-3 / MTP；接受长度为核心指标；一实例一 drafter，按 `x-task-type` 选实例）；§5.1 档位预算计入 drafter | 待写 |
| 7 | vLLM preset 未开 `--enable-prefix-caching`；SGLang 只是"预留"，但它对 agent 流量（RadixAttention）和投机解码更合适 | 多轮 agent 每轮全量预填充 | §4.2 加前缀缓存参数并写明与会话粘性的依赖；§4.4 从预留改为 GPU 池默认候选，补 launch 参数 | 待写 |
| 8 | 24GB 档强池 5K 上下文对 agent 不够，档位表按硬件分不按用法分 | 客户按表买了机器跑不了 agent | §5.1 已加"按用法分档"说明 | **本次已改** |
| 9 | 弱池 llama.cpp 单槽位、Ollama 类引擎近串行，未在容量表中区分引擎并发模型 | 容量估算失真 | §7.1 增加"每引擎并发模型"表（连续批处理 / 固定并行数 / 单槽） | 待写 |

## 3. Harness 与 token 经济

| # | 缺口 | 影响 | 改哪 | 状态 |
|---|---|---|---|---|
| 10 | 对 harness 只有 provider 配置规范，没有"省 token 契约"：前缀稳定顺序、工具输出裁剪、压缩阈值、子任务隔离、按任务关思考 | 每任务成本无上限 | 第三部分 §2 新增 §2.5 harness 侧 token 规范（草案 §4 的表）；§2.2 增加 `x-task-type` 打标扩展 | 待写 |
| 11 | 改写（rewriter）没有位置；也没有"不得改动已发出历史"的硬约束 | 一旦引入压缩就可能打碎缓存、反而涨成本（文献实测 +6.8%） | §2.5 内写四层改写与两条硬规则；工具输出只用规则瘦身，不用通用压缩器 | 待写 |
| 12 | 请求契约没有 `x-session-id` / `x-task-type` | 粘性和任务类型无处承载 | 第二部分 §1.1 已加请求头表 | **本次已改** |

## 4. 数据、指标与评测

| # | 缺口 | 影响 | 改哪 | 状态 |
|---|---|---|---|---|
| 13 | 指标体系（§8.1）只有可用性与容量，没有"每个成功任务的成本"，没有接受长度、缓存命中率 | 无法证明任何优化有效 | §8.1.1 增加任务级指标：token/任务、GPU 秒/任务、成功率、缓存命中率、接受长度；定义"成功" | 待写 |
| 14 | 没有轨迹日志契约：记什么（请求摘要、路由、结果、token、延迟）、存哪、留多久、脱敏 | RL 路由器、EAGLE 头、GEPA 都没有训练数据 | 第三部分 §6.1 文件契约增加 `logs/trajectories.jsonl` 规范，含 ε 探索标记 | 待写 |
| 15 | 没有重放机：同一批真实轨迹反复重放做 A/B | 每次改动只能凭感觉 | §6.3 CLI 增加 `cluster replay`；§8 增加评测流程 | 待做（阶段 1） |
| 16 | 新模型接入（§4.6）只讲 preset，没有评测集、影子评估、模型 × 任务类型表 | "自动选模型"无依据 | §4.6 扩为"模型评测与接入"：任务集、指标、影子评估比例 | 待写 |

## 5. 组网、运维与可靠性

| # | 缺口 | 影响 | 改哪 | 状态 |
|---|---|---|---|---|
| 17 | 跨网络只有 `link-gpu`（Hub 主动 SSH 到节点），要求 Hub 能登录节点；没有节点主动连 Hub 的反向隧道，没有 overlay 指引 | 客户机房、云容器、异地设备接不进来 | 第一部分 §7 新增"网络拓扑"节（同网段 / 跨网段 / 跨网络三种）；第三部分 §6 新增 connector 组件规范（rathole/chisel 内嵌，节点出站） | 待写 |
| 18 | 只有 Hub 有看门狗；`join` 进来的节点引擎崩了没人重启 | 非 Hub 节点不自愈 | §6 增加节点侧 systemd/launchd 守护规范；看门狗只管路由 | 待写 |
| 19 | Hub 单点未作为明确取舍写进非目标 | 读者以为有高可用 | 第一部分 §8 加一条 | 待写 |
| 20 | 没有升级/滚动重启流程（换模型、换引擎版本时先摘后换） | 换模型即停服 | §6.3 增加 `cluster drain <node>` / `upgrade` | 待写 |
| 21 | 监控只到"建议"，无 Prometheus 抓取配置；`/metrics` 未接 | 容量和成本指标拿不到 | §8.1.2 落地抓取清单（vLLM、SGLang、Switchyard、LiteLLM 都原生出 `/metrics`） | 待写 |
| 22 | 容器环境读不到 GPU 遥测（PAIR 实测 `gpus: null`），我们的探测同样依赖 `nvidia-smi` 命令 | 云上容器压力信号缺失 | §6 探测改为 NVML 库或读 `/metrics`，不 exec 命令 | 待写 |
| 23 | 模型权重不锁 revision、下载无校验（§4.6 提到应锁，安装脚本未做） | 复现性 | `bin/install.sh` 与 §4.6 补 revision 与 sha256 | 待做 |

## 6. 已被实践推翻、需要改写的表述

- 第一部分 §4 "加一台设备等于加一个副本，下一个请求起生效"：已改为"当前只做故障转移"，待 #2 落地后再改回。
- 第一部分 §5.1 "接入层和网关对在途请求没有实质上限"：对，但要补一句"上限在引擎工位数，由显存决定"，并给出并发扫描的测法（§7.2）。
- 第三部分 §2.1 "接入层要薄"：仍成立，但"薄"不等于"不管 token"——省 token 的大头恰恰在个人端（#10）。

## 7. 处理顺序

先做能立刻产生数据的：#13 指标、#14 日志契约、#15 重放机（阶段 1）；然后 #3 外壳 + #2 池内分发 + #1 决策器（阶段 2–3）；#6/#7 投机解码与前缀缓存（阶段 5）；#17/#18 组网与节点守护随客户部署需要推进；#19–#23 是文档与脚本层面的小修，顺手做。
