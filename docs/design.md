...
一个网关进程把 OpenAI 兼容的请求分发到多台机器上的推理引擎，云端 API 作为
...
接入层和网关对在途请求没有实质上限（每请求一对连接和一个挂起协程），不计入。
...
层与层之间只有 HTTP。没有共享内存、共享文件或进程内调用跨过层边界；控制平面
的文件接口只在 Hub 内部。因此每一层可以放在任何设备上，同机和分机只差一个
网络地址。

| 边界 | 通信 | 进程关系 | 宿主关系 |
|---|---|---|---|
| 用户 → 接入层 | HTTP | 每用户一个客户端进程 | 用户自己的设备或任意主机 |
| 接入层 → 调度层 | HTTP（OpenAI/Anthropic 格式） | 独立进程 | 同机或分机 |
...
- 在网关内执行工具。工具由客户端（如 Pi）执行，网关只透传 `tool_calls`。
