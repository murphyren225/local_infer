...
「同时处理」以引擎的工位数计（一个工位 = 一个请求专属的 KV cache 空间）。
超过工位数的请求在引擎门口排队，不失败。

| 部署形态 | 强池工位 | 弱池工位 | 同时在算的请求 | 适合规模 |
|---|---|---|---|---|
| 标准盒（单 24GB 卡，弱池为卡上 1.7B 分片） | 2 | 8 | 10 | 10–20 人交互式使用 |
| Mac Hub + 远端 4090（当前验证部署） | 2 | 1（llama.cpp 单槽位） | 3 | 单人/演示 |

接入层与网关对在途请求没有实质上限（每请求一对连接加一个挂起协程，成本近零），
不计入。

### 4.2 一个请求各环节耗时（200 token 回答，实测）

| 环节 | 耗时 | 说明 |
|---|---|---|
| 接入层转发 | < 5 ms | 包装为 OpenAI 格式并转发 |
| 网关路由决策 | < 1 ms | 查路由表与会话粘性表 |
| 判定器（仅裁决轮） | 1–2 s | 一次弱池短调用；不是每轮都发生 |
| 排队等待 | 0（有空位）～一轮完成时间（无空位） | 一轮 = 前一个请求生成完毕，7–15 s |
| 跨设备网络（隧道） | 50–100 ms | 同局域网可忽略 |
| 首 token | 0.15–0.17 s | 弱池 / 强池实测 |
| 生成 200 token | 弱池 vLLM 约 7 s（4 路并发时）；强池约 10 s（2 路并发时）；Mac CPU 约 14 s | 并发越高单路越慢、总吞吐越高 |

典型端到端：弱池直答约 8 s，经强池约 12 s，排队一轮再加 7–15 s。

### 4.3 每台设备每小时产能

单位为「个回答」，按每个回答 200 token 计算。换算：
`每分钟回答数 = 每秒 token 数 × 60 ÷ 200`。

| 设备 / 池 | 聚合吞吐（token/秒） | 每分钟回答数（个） | 每小时回答数（个） |
|---|---|---|---|
| 4090D 强池（32B） | 38 | 约 11 个 | 约 660 个 |
| 4090D 弱池分片（1.7B，vLLM） | 108 | 约 32 个（扣判定器开销后约 27 个） | 约 1600 个 |
| Mac i9 弱池（1.7B，llama.cpp） | 14 | 约 4 个 | 约 250 个 |

一台标准盒（强池 + 弱池分片）每小时约 2200 个回答；产能与回答长度成反比，
1000 token 的长回答按五分之一计。

### 4.4 时间策略

- **目标**：弱池类任务端到端 < 10 s，强池类 < 30 s，排队不超过一轮。
- **超时**：客户端 300 s，网关到引擎 600 s。请求不会被主动丢弃，超时由计时器
  终止连接。
- **保强池队列短**：分诊把约八成流量引向弱池，是当前唯一的排队控制手段。
- **扩容触发**：任一池等待队列持续 5 分钟非空，或 p95 延迟超目标两倍 → 池内加
  同型节点（架构零改动）。
- **已知限制**：无过载拒绝（不返回 429），无租户配额。超容量时的表现是全体变慢，
  不是部分被拒。

各部件的并发机制、上限与超载表现：

| 部件 | 靠什么并发 | 能扛多少 | 超载时的表现 |
|---|---|---|---|
| 接入层 | 单进程异步 I/O | 数百连接，远超需求 | 不会发生 |
| 网关 | 无状态异步代理 | 非瓶颈；`--workers N` 可多进程 | 不会发生 |
| 判定器 | 每次裁决占一次弱池调用 | 弱池容量的 10–20% | 随弱池排队 |
| 弱池（vLLM） | 连续批处理 | 8 工位 | 排队，延迟上升 |
| 弱池（llama.cpp） | 单槽位（`--parallel N` 可增） | 1 | 串行排队 |
| 强池（vLLM） | 连续批处理 | 1–2 工位 | 排队，延迟上升 |
| 云端 | 供应商侧 | 视配额 | 供应商返回 429 |
...
控制平面的文件接口只存在于 Hub 内部，不跨层。由此每一层、每一台设备都可以
独立放置、独立扩缩、独立失效：任何一层放在任何设备上都成立，同机与分机
只是网络地址的差别。

| 边界 | 通信 | 进程关系 | 宿主关系 |
|---|---|---|---|
| 用户 → 接入层 | HTTP | 每用户一个客户端进程 | 用户自己的设备或任意主机 |
| 接入层 → 调度层 | HTTP（OpenAI/Anthropic 格式） | 独立进程 | 可同机可分机 |
| 调度层 → 资源层 | HTTP | 独立进程 | 通常分机；同机时按 §6.3 隔离 |
| 调度层 → 云端 | HTTPS | 外部服务 | 外网 |

### 6.2 接入层（harness）的两条放置约束
