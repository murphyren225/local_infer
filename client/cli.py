"""client CLI — the personal side, one per person.
  ...
  client pi   [pi args…]          open the Pi harness against the cluster
...
def cmd_setup(args) -> int:
    hub = _hub(args)
    _save(hub=hub)
    out = pi_setup.setup(hub + "/v1")
    print(f"  ✓ Pi provider 'home' → {hub}/v1   ({out['models']})")
...
def cmd_up(args) -> int:
    ...
    if (pid := _web_pid()):
        print(f"  already running (pid {pid}) at http://127.0.0.1:{_config().get('port', args.port)}")
        return 0
    HOME.mkdir(parents=True, exist_ok=True)
    env = {**os.environ, "CLIENT_GATEWAY": hub, "CLIENT_PORT": str(args.port)}
    ...
    for _ in range(20):
        if gateway.healthy(f"http://127.0.0.1:{args.port}/api/config"):
            break
        time.sleep(0.25)
    print(f"  ✓ local service http://127.0.0.1:{args.port}  →  {hub}   (pid {proc.pid}, log {LOG})")
    return 0
...
def cmd_ask(args) -> int:
    r = gateway.chat(_hub(args), args.lane, args.prompt, max_tokens=args.max_tokens)
    print(r["content"])
...
def cmd_batch(args) -> int:
    ...
    def one(p):
        try:
            r = gateway.chat(hub, args.lane, p, max_tokens=args.max_tokens)
...
def main(argv=None) -> int:
    ...
    s = add("pi", cmd_pi, hub=False); s.add_argument("rest", nargs=argparse.REMAINDER)
